...
class ElasticsearchManager(BaseComponentManager):
    ...
    def manager_status(self, container_name):
        command = "docker ps | grep %s | awk '{print $1}'" \
                  % container_name 
        ret = os.popen(command)
        container_id = ret.read().strip()
        logging.info('container id :%s, command:%s'
    ...
    def __check(self, container_id):
        ...
        cmd = self.validate_cmd if self.validate_cmd else \
              "curl -d '' 'http://127.0.0.1:9999/admin/conf'"
        ...
        if not flag in status:
            result = False
            component_restart_cmd = nsenter + \
                'service %s restart' % self.component_manager
